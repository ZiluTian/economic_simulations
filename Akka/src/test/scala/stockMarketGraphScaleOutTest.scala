...
object stockMarketGraphScaleOutTest extends scaleOutTest with App {
    import BSPModel.example.stockMarket.ConditionActionRule._
    ...
    def gen(machineId: Int, totalMachines: Int): IndexedSeq[Actor] = {
        (0 until localScaleFactor).map { id =>
            val traders: Int = baseFactor / localScaleFactor - 1
            val agent = new stockMarketPartition(id, traders)
            // market agent is the first in the world array per partition
            if (id == 0){
                agent.connectedAgentIds = (1 until localScaleFactor * totalMachines).map(_.toLong * (baseFactor / localScaleFactor))
            }
            agent
        }
    }
